...
static inline void cpu_probe_ver2(struct cpuinfo_csky *c, unsigned int cpu)
{
	...
	c->cache_size |= CPUID_ICACHE_8K | CPUID_DCACHE_8K;
	c->sram_size |= CPUID_DCACHE_8K + CPUID_DCACHE_4K;
...
__cpuinit void cpu_report(void)
{
	...
	if(c->fpu_id) {
		printk(KERN_INFO "FPU revision is: %s\n",
				(c->fpu_id == CPUID_FPU_V1 ? "a coprocessor(V1)" : "CPU unit(V2)"));
	}
#endif
}
